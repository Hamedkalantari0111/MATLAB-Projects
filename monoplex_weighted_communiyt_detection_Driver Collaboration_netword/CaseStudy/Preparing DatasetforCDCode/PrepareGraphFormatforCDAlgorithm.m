...
% clc
% clear all
% InputData=dlmread('CaseStudy_SourceTargetWeightWRank.txt');
...
% for i=1:Row
%     InputData(i,5)=i; % InputData(i,5)= shenase edge dataset vorodi
% end
% save InputData.mat 
% %%
% clear all
...
%         DNet(k,j+5)=row;  %==============================================DNet(:,6) means DNet first node Index and %DNet(:,7) means DNet Second node Index
%     end
% end
...
[row, col]=find(DNet(:,6)==NumofNodes);
...
for i=1:size(DNet_Senario1,1)
    DNet_Senario1(i,8)= find(CC(:,1)==DNet_Senario1(i,1));
    DNet_Senario1(i,9)= find(CC(:,1)==DNet_Senario1(i,2));
end
DNetSenario1=[(DNet_Senario1(:,1:7)),(DNet_Senario1(:,8:9))]; % Final MAPPED Inputs for Driver Network in Modeling (MY MAPPED CASE STUDY DATA)!!!!!!!!!!!!!!
k=1;
for i=1:size(DNetSenario1,1)
    for j=1:size(DNetSenario1,1)
        if i~=j
            if DNetSenario1(i,8)== DNetSenario1(j,9) && DNetSenario1(i,9)==DNetSenario1(j,8) % find the Duplicate records in the Networc (because of Undircted Graph)
...
finaldataset=[DNetSenario2(:,1:5),DNetSenario2(:,8:9)];
