    ...
    if size(NC,2)>2
        ...
        for m=1:size(NC,2)
            ...
            if  M_temp(m,w)>M(w,1)
                ...
                if XX==ZZ % in kar baraye hazfe Inpute feli az entekhabe inpute jadid dar marhale baad ast!!!!!!!!!!!!!
                    for u=1:size(LC,2)
        ...
        Inputtt=zeros(1,1);
        Inputtt=Inputt; % inpute jadid dar dore bad. yani inputi ke ta ghabl az in dashtim kar mikardim ra az data base hazf mikonim
        Inputt=zeros(1,1);
...
%     bg = biograph(Adja_Mat);  % make biograph object
...
[EQ_Local]=EQ_Measure_Func(Input,Nodes,local_Community,Commun_size,Adja_Mat);% calculae the Q measure for the Local communities in Layer#1
No=1;
            ...
            if Adja_Mat(local_Community(i,j),local_Community(i,k))==1
                for l=1:size(Input,1)
                    if local_Community(i,j)==Input(l,1) && local_Community(i,k)==Input(l,2)
                        Loc_edge_index(i,No)=Input(l,4); % edge indecex of each local community
                        No=No+1;
                        break
                    end                    
                end
...
for i=1:size(local_Community,1)
    for j=1:size(local_Community,1)
        ...
        Subscript_edge(i,j)=sum(Subscr_edge(i,j,:)); % yalhaye eshteraki beine har 2 local community
    end
end
...
[EQ_phase2]=EQ_Measure_Func(Input,Nodes,Final_Community,Final_Community_size,Adja_Mat);% calculae the Q measure for the Local communities in Layer#1
save LocalCommunities_Phase2.mat 
clear all
clc
load ('LocalCommunities_Phase2.mat','Final_Community','Final_Community_size','Input','Nodes','Adja_Mat','EQ_phase2')
    %======================part3: refining=========================
...
[EQ_phase3]=EQ_Measure_Func(Input,Nodes,FinalCommunity_phase3,FinalCommunity_phase3_size,Adja_Mat)% calculae the Q measure for the Local communities in Layer#1
save LocalCommunities_Phase3.mat 
clear all
load ('LocalCommunities_Phase3.mat','FinalCommunity_phase3','Final_Community2','M_Measure','Input','Nodes','Adja_Mat','FinalCommunity_phase3_size','NodesExistinAllComs','Outlayer','EQ_phase3')
load ('LocalCommunities_Phase2.mat','EQ_Local','EQ_phase2')
%=========================== plloting the graph======================
for i=1:size(FinalCommunity_phase3,1)
    k=1;
