function [WOS_NCA,NCA,NCA_p1,NCA_p2]=WOSFunc_New(LC,Commun_size,Comm_loc_edge_index,Subscript_node,Alfa,Subscript_edge,Gama,Nodes_NotinCom_i_incom_j,AdjaMat,NodesInCommunity,CCC)
              ...
              for k=1:size(NodesInCommunity,2)-sum(NodesInCommunity(i,:)==0)
                  for o=1:size(NodesInCommunity,2)-sum(NodesInCommunity(j,:)==0)
                      if  CCC(i,k,j)==0 || CCC(j,o,i)==0
                          AAAA=AAAA+AdjaMat(NodesInCommunity(i,k),NodesInCommunity(j,o));
                      end
                  end
              end
          end
