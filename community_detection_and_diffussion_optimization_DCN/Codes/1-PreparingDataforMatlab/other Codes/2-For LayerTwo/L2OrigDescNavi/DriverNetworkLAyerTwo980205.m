% %=====================================================================
% %                          Preparing Second Layer Data (Edge= ham mabda va ham maghsad va ham mashin)
% %=====================================================================
% clc
% clear all
% InputData=dlmread('DnetL2OrigDescNavi.txt');
...
% save Input_L2OrigDescNavi.mat % Layer 1 is the Layer of the Drivers Network of which the edge is the "frighterID"
% clear all
% clc
% load ('DriverNetLayerOneFinal.mat','ResultDataLayerOne')
% load ('Input_L2OrigDescNavi.mat','InputData','Row','Column')
...
%                     NewInputData(k,1:2)=InputData(i,1:2); % New input based on the layer one nodes
%                     k=k+1;
%                 end
%             end
%         end
...
%     end
% end
% save InputNetwork_L2OrigDescNavi.mat % Layer 1 is the Layer of the Drivers Network of which the edge is the "frighterID"
% clear all
% load ('InputNetwork_L2OrigDescNavi.mat','DNet','A')
...
%     end
% end
% xlswrite('L2OrigDescMatlabPrepared.xls',DNetSenario2(:,3:4)) %clmn1=IDfrstDriver,clmn2=IDScndDriver,clmn3=SourceCode(FirstDriverMappedID),clmn4=TargetCode(SecondDriverMappedID)
% Length= size(CC,1);
% save DNetL2Result.mat
% clear all
% load ('DNetL2Result.mat','DNetSenario2', 'Length')
...
%             ResultDataLayerTwo(i,3:4)=DNetSenario2(j,1:2); % Note: clmn3=IDfrstDriver,clmn4=IDScndDriver
%         end
%     end
% end
% save DriverNetLayerTwoFinalOrigDescNavi.mat % 
%%=====================================================================
clear all
clc
load ('DriverNetLayerTwoFinalOrigDescNavi.mat','ResultDataLayerTwo')
load ('TotalNetworkRelationValue.mat','TotalRowUniq')
%calculating the lower and upper bound of fuzzy relation value for the source node
for i=1:size(ResultDataLayerTwo,1)
...
for i=1:size(ResultDataLayerTwo,1)
    for j=1:size(TotalRowUniq,1)
        if ResultDataLayerTwo(i,3)==TotalRowUniq(j,2) && ResultDataLayerTwo(i,4)==TotalRowUniq(j,1)
            ResultDataLayerTwo(i,7:8)=TotalRowUniq(j,3:4); % clmn3=IDfrstDriver,clmn4=IDScndDriver 
            % and clmn 7 = Second Driver lower bound of fuzzy relation value ,clmn 8 = Second Driver upper bound of fuzzy relation value
            break;
        end
    end
end
